...
export const handleRoomEvents = (io: Server, socket: Socket) => {
    ...
    socket.on("joinRoom", async ({ roomId, name }: { roomId: string, name: string }) => {
        try {
            // 1. Generate peerId
            ...
            console.log(`👤 User [${name}] joined room [${roomId}] with peerId: ${peerId}`);

        } catch (error) {
            console.error("🔴 Error in joinRoom flow:", error);
